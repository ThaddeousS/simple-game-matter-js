...
export class ScaleTool extends TransformTool {
  constructor(editor, entity) {
    ...
    this.handleSize = 20; // Increased even more for better interaction
  }
  ...
  onMouseDown(e, worldPos) {
    ...
    if (scaleHandle) {
      ...
      this.initialScales = this.entities.map((entity) => {
        if (entity.config.shape === "circle") {
          return { entity, radius: entity.config.radius };
        } else {
          return {
            entity,
  ...
  onMouseMove(e, worldPos) {
    if (!this.isDragging || this.entities.length === 0 || !this.initialScales)
      return;
    ...
    for (let initialState of this.initialScales) {
      const entity = initialState.entity;

      if (entity.config.shape === "circle") {
        const scaleFactor = 1 + (dx + dy) * 0.01;
        const newRadius = Math.max(5, initialState.radius * scaleFactor);
        Body.scale(
          entity.body,
        ...
        switch (this.scaleMode) {
          case "uniform":
            ...
            newWidth = Math.max(10, initialState.width * avgScale);
            newHeight = Math.max(10, initialState.height * avgScale);
            break;
          case "x":
            // Scale along rotated X-axis
            newWidth = Math.max(10, initialState.width + localDX * 2);
            break;
          case "y":
            // Scale along rotated Y-axis
            newHeight = Math.max(10, initialState.height + localDY * 2);
            break;
        }

        // Apply scale to body
        const scaleX = newWidth / entity.config.width;
        const scaleY = newHeight / entity.config.height;

        Body.scale(entity.body, scaleX, scaleY);
  ...
  getScaleHandle(worldPos) {
    ...
    const hitRadius = 25; // Increased from 15 for better interaction

    // For multi-select or single entity, we'll use a simplified approach
    // Check for uniform scale handle at bottom-right
    if (this.entity && this.entity.config.shape === "circle") {
      const dx = worldPos.x - (pos.x + this.entity.config.radius);
      const dy = worldPos.y - pos.y;
      if (Math.sqrt(dx * dx + dy * dy) < hitRadius) return "uniform";
    } else if (this.entity) {
      ...
      // Corner handle (uniform scale) - in local space with larger hit area
      const cornerDX = localX - hw;
      const cornerDY = localY - hh;
      if (Math.sqrt(cornerDX * cornerDX + cornerDY * cornerDY) < hitRadius)
        return "uniform";

      // Right edge (X scale) - in local space with larger hit area
      if (Math.abs(localX - hw) < hitRadius && Math.abs(localY) < hh)
        return "x";

      // Bottom edge (Y scale) - in local space with larger hit area
      if (Math.abs(localY - hh) < hitRadius && Math.abs(localX) < hw)
        return "y";
    }
