...
export class ScaleTool extends TransformTool {
    ...
    getScaleHandle(worldPos) {
        ...
        if (this.entity.config.shape === 'circle') {
            ...
            if (Math.sqrt(cornerDX * cornerDX + cornerDY * cornerDY) < 15) return 'uniform';
    ...
    renderWidget(ctx, camera) {
        ...
        if (this.entity.config.shape === 'circle') {
            ...
            this.renderHandle(ctx, screenX + radius, screenY, scale, '#ff00ff', this.handleSize);
        } else {
